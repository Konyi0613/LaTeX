\chapter{Motions}\label{ch:vim-motions}

A \vocab{motion} is any command that moves the cursor. Motions matter far beyond
navigation, because \autoref{ch:vim-grammar} will feed every one of them to an
...
\section{Characters and Lines}\label{sec:vim-charline}
...
\section{Words and WORDs}\label{sec:vim-words}
...
\section{Finding a Character on the Line}\label{sec:vim-findchar}
...
\begin{eg}
	...
	the parentheses' contents, \key{d} \key{i} \key{(} (\autoref{sec:vim-textobj}).
\end{eg}

\section{Around the File, and Scrolling}\label{sec:vim-file}
...
\section{Search as a Motion}\label{sec:vim-search}
...
\section{Jumps, Marks and the Change List}\label{sec:vim-jumps}
