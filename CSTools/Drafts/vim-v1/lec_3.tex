\chapter{The Grammar: Operators and Text Objects}\label{ch:grammar}
...
\begin{eg}
	Combining the operators above with the motions of \autoref{ch:motions}:
	\begin{keys}[0.16]
...
\section{Text Objects}\label{sec:textobj}
...
\begin{remark}
	Neovim's \texttt{mini.ai} plugin (shipped with LazyVim, \autoref{ch:setup})
...
\section{Repeat: the Dot Command}\label{sec:dot}
...
\begin{tryit}
	...
	\dots{} Then do the same thing with \key{q} \key{q} (\autoref{sec:macros}) and
	compare.
\end{tryit}
