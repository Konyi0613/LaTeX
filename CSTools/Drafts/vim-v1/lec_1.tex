\chapter{The Modal Mindset}\label{ch:modes}
...
precise in \autoref{ch:grammar} --- it is the single idea worth taking away from
this note.
...
\begin{definition}[Insert mode]
	...
	Entered with \key{i}, \key{a}, \key{o} and friends (\autoref{sec:insert-entry}),
...
\section{Reading the Manual}\label{sec:help}
...
\begin{keys}[0.30]
	...
	\cmd{'clipboard'}                      & Set to \cmd{unnamedplus} to share yanks with the system clipboard (\autoref{sec:registers})                                            \\
\end{keys}
...
spelling is \cmd{vim.opt.number = true} (\autoref{sec:lua-config}).
