...
The word ``equivalent'' has already been used twice in quotation marks. Here is what it means.

\begin{definition}[Equivalent rates]
	\label{def:equivalent-rates}
	The \vocab{accumulation factor} $A(n)$ of a quoting convention is the amount one dollar grows to under it after $n$ years:
	\[
		A(n) = \begin{cases}
			e^{rn},                                   & r \text{ compounded continuously,}       \\[6pt]
			\Bigl(1+\dfrac{r}{m}\Bigr)^{mn}, \quad & r \text{ compounded $m$ times per annum.}
		\end{cases}
	\]
	Two quoted annual rates are \vocab{equivalent} if they turn one dollar into the same amount of money at every horizon --- that is, if their accumulation factors agree as functions of $n$.
\end{definition}

\begin{note}
	Checking one year is enough.
	Each accumulation factor above is a pure power of its own one-year factor, $A(n)=A(1)^{n}$, so $A_{1}(1)=A_{2}(1)$ already forces $A_{1}(n)=A_{2}(n)$ for every $n\ge 0$.
	Equivalently: two rates are equivalent exactly when they share the same \vocab{effective annual rate} $A(1)-1$ of \autoref{sec:rate-frequency}.
\end{note}
...
\begin{note}
	...
	It will be justified later in the course from ``sound'' principles rather than assumed --- see \autoref{thm:pv-justified}, where the only principle used is that there is no free lunch.
\end{note}
...
\begin{remark}
	...
	例如今天付 100、一年後拿回 108.16，同一筆現金流可報價為 $8.16\%$（$m = 1$，即\vocab{有效年利率}（effective annual rate, EAR））、$8.00\%$（BEY）或 $7.87\%$（MEY）：$m$ 愈大，達成同樣成長所需的\vocab{名目年利率}（nominal rate）愈小。
