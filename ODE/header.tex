...
% \slopemark{colour}{x}{y}{slope}{kx}{ky} — one tangent segment of a direction
% field, drawn inside a pgfplots `axis'.  kx/ky are the cm-per-unit scales of the
% two axes; the segment is normalised so that every mark has the same *visual*
% length regardless of how steep it is.  The \edef is essential: pgfplots defers
% the drawing commands of an axis until \end{axis}, by which point the local
% macros of the surrounding \foreach are long gone, so the numbers must be baked
% into the \draw before it is handed over.
\newcommand{\slopemark}[6]{%
	\pgfmathsetmacro{\SMnorm}{sqrt((#5)*(#5)+((#6)*(#4))*((#6)*(#4)))}%
	\pgfmathsetmacro{\SMxa}{(#2)-0.32/\SMnorm}%
	\pgfmathsetmacro{\SMxb}{(#2)+0.32/\SMnorm}%
	\pgfmathsetmacro{\SMya}{(#3)-0.32*(#4)/\SMnorm}%
	\pgfmathsetmacro{\SMyb}{(#3)+0.32*(#4)/\SMnorm}%
	\edef\SMdraw{\noexpand\draw[#1, line width=0.5pt]
		(axis cs:\SMxa,\SMya) -- (axis cs:\SMxb,\SMyb);}%
	\SMdraw
}
...
% stack does not survive \vsplit: the continuation segment starts with whatever
% colour was set last, which is the `colback' just painted for that segment.
% Its body text is then drawn in the background colour, i.e. invisibly — the
% text is still in the PDF and pdftotext finds it, but nothing is readable.
% LuaTeX's native \pdfcolorstack is split-safe, so the same source is fine
% there.  Keep such boxes whole under XeTeX instead; they then move to the next
% page as a unit, exactly as the unbreakable `theorem box' already does.
\ifXeTeX
...
\tcbset{
	...
	ques box/.style={
		...
		splitsafe, left=10pt, right=10pt
	}
}
