...
% `splitsafe' — kept as a name so existing boxes keep working; it is now just
% `breakable'.  It used to expand to `unbreakable' under XeTeX, because older
% tcolorbox releases lost the colour across \vsplit and drew the continuation
% segment's body text in the background colour.  Current tcolorbox (TeX Live
% 2024) handles this correctly under XeLaTeX — verified with colback + xeCJK
% text spanning a page break — so all these boxes now split normally.
\tcbset{splitsafe/.style={breakable}}

\tcbset{
	theorem box/.style={
		enhanced, breakable, arc=9pt, outer arc=10pt,
