...
\begin{proof}
    We first give a claim:
     ...
     \begin{claim} \label{clm: Sn bounded (wide)}
        \(
            0 \le \sum_{k=n}^m (-1)^{k+1} g_k(x) \le g_n(x) 
        \) for all \(n,m \in \mathbb{N} \) and \(x \in T\).  
     \end{claim}   
     \begin{explanation}
        If \(m < n\), then \(\sum_{k=n}^m (-1)^{k+1}g_k(x) = 0 \), so it is true. If \(m \ge n\), then note that
        \begin{align*}
            \sum_{k=n}^m (-1)^{k+1} g_k(x) &= g_n(x) - g_{n+1}(x) + \dots + (-1)^{m} g_m(x) \\
             &= g_n(x) - (g_{n+1}(x) - g_{n+2}(x)) - \dots \le g_n(x)
        \end{align*}
        since \(g_i (x) - g_{i+1}(x) \ge 0\). Also, we can check that
        \begin{align*}
            \sum_{k=n}^m (-1)^{k+1} g_k (x) &= g_n(x) - g_{n+1}(x) + \dots + (-1)^m g_m(x) \\
        ...
        so we're done.
     \end{explanation}
     Now by \autoref{clm: Sn bounded (wide)} we know \(S_n\) is bounded for all \(n \in \mathbb{N} \), and if we fix \(x_0 \in T\) and suppose 
     \[
     ...
     \begin{align*}
        ...
        &= \lim_{m \to \infty} \sum_{k=n+1}^m (-1)^{k+1} g_k(x) \\
        &\le \lim_{m \to \infty} g_{n+1}(x) = g_{n+1}(x) < \varepsilon    
     \end{align*}   
...
\begin{proof}
    ...
    Also, suppose 
    \[
        p(n) = \left( 1 - \frac{n+1}{n+2} \right)\left( \frac{n+1}{n+2} \right)^{n+1} = \frac{(n+1)^{n+1}}{(n+2)^{n+2}},  
    \] for \(n \ge 0\), then suppose
    \[
        h(n) = \ln (p(n)) = (n+1) \ln (n+1) - (n+2) \ln (n+2),
    \] we know 
    \[
        h^{\prime} (n) = \ln (n+1) + 1 - \ln (n+2) - 1= \ln \left( \frac{n+1}{n+2} \right) < 0,
    \] which means \(\ln (p(n))\) is decreasing when \(n \ge 0\), which means \(p(n)\) is decreasing when \(n \ge 0\). Hence, for all \(\varepsilon > 0\), we can pick some \(N > 0\) s.t. 
    \[
       p(N) = \frac{(N+1)^{N+1}}{(N+2)^{N+2}} < \varepsilon,
