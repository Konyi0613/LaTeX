...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \[
      \begin{dcases}
        A \subseteq S \subseteq \overline{A} \\
        A \subseteq T \subseteq \overline{S}, 
      \end{dcases}
    \]
    and we want to show that \(A \subseteq T \subseteq \overline{A} \). Since \(A \subseteq S \subseteq T\), so we have \(A \subseteq T\). Now we want to show \(T \subseteq \overline{A} \). If there exists \(x \in T\) but \(x \notin \overline{A} \), then we know 
    \[
      \forall r > 0, \ \begin{dcases}
        B(x, r) \cap S \neq \varnothing \\
        B(x, r) \cap (X\setminus S) \neq \varnothing
      \end{dcases} \quad \text{ since } T \subseteq \overline{S}. 
    \] Also, we know there exists \(r^{\prime} > 0\) s.t. \(B(x, r^{\prime} ) \cap A = \varnothing \). Now since \(A \subseteq S\), so we know 
    \[
      B(x, r^{\prime} ) \cap (S \setminus A) \neq \varnothing .
    \]   
    Besides, note that \(S \subseteq \overline{A} = A \cup \partial A \), so \((S \setminus A) \subseteq \partial A\), so we know there exists \(y \in \partial A\) s.t. \(y \in B(x, r^{\prime} )\). Note that \(d(y,x) < r^{\prime} \), and we define \(\varepsilon = r^{\prime} - d(y, x) > 0\). Also, we have 
    \[
      \forall r^{\prime\prime} > 0, \begin{dcases}
        B(y, r^{\prime\prime} ) \cap A \neq \varnothing \\
        B(y, r^{\prime\prime} ) \cap (X \setminus A) \neq \varnothing.
      \end{dcases}
    \]  Hence, we know for some \(a \in A\), \(a \in B(y, r^{\prime\prime} )\) for all \(r^{\prime\prime} > 0\), which means \(d(y, a) < r^{\prime\prime} \). If we pick \(r^{\prime\prime} \) s.t. \(0 < r^{\prime\prime} < \varepsilon \), then we have 
    \begin{align*}
      d(a, x) &\le d(a, y) + d(y, x) \\
              &< \varepsilon + d(y, x) \\
              &= r^{\prime} - d(y,x) + d(y,x) = r^{\prime} .
    \end{align*}      
    Hence, we have \(a \in B(x, r^{\prime} )\), but \(a \in A\), so \(a \in B(x, r^{\prime} ) \cap A = \varnothing \), which is a contradiction. Thus, we know for every \(x \in T\), we must have \(x \in \overline{A} \), which means \(T \subseteq \overline{A} \), and we're done.      
    \item [(b)] Since \(S \subseteq \overline{A} \), so for all \(x \in S\) and \(r > 0\), we know \(B(x, r) \cap A \neq \varnothing \). We want to show that for all \(x \in B\), we have \(B(x, r) \cap A \cap B \neq \varnothing \) for all \(r > 0\). Now suppose \(x \in B \subseteq S\).  Since \(B\) is open in \(S\), so there exists \(O \subseteq X\) s.t. \(O\) is open and \(B = O \cap S\). Note that for all \(x \in B \subseteq S\), there exists \(r_1 > 0 \) s.t. \(B(x, r_1) \subseteq O\). Hence, we have \(B(x, r_1) \cap S \subseteq O \cap S = B\). Also, since we know \(A \subseteq S\), so 
    \[
      B(x, r_1) \cap A \subseteq B(x, r_1) \cap S \subseteq B,
    \] which shows that \(B(x, r_1) \cap A \cap B \neq \varnothing \). Now if \(0 < r_2 < r_1\), then since \(B(x, r_2) \subseteq B(x, r_1)\), so we have 
    \[
      B(x ,r_2) \cap S \subseteq B(x, r_1) \cap S \subseteq B.
    \] Also, we still have \(B(x, r_2) \cap A \neq \varnothing\), and similarly we have 
    \[
      B(x, r_2) \cap A \subseteq B(x, r_1) \cap S \subseteq B,
    \] which shows \(B(x, r_2) \cap A \cap B \neq \varnothing \). Now if \(r_3 > r_1\), then since \(B(x, r_1) \subseteq B(x, r_3)\), and we have shown that \(B(x, r_1) \cap A \cap B \neq \varnothing \), so we have 
    \[
      \varnothing \neq B(x, r_1) \cap A \cap B \subseteq B(x, r_3) \cap A \cap B.
    \]    Hence, for all \(r > 0\), we know \(B(x, r) \cap A \cap B \neq \varnothing \), and we're done.  
    \item [(c)] By (b), we know \(B \subseteq \overline{A \cap B} \). Also, we always have \(A \cap B \subseteq B\), so we have \(A \cap B \subseteq B \subseteq \overline{A \cap B} \), which means \(A \cap B\) is dense in \(B\). Now since \(B\) is dense in \(S\), so by (a) we know \(A \cap B\) is dense in \(S\).       
  \end{itemize}
\end{proof}