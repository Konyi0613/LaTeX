...
In fact, one can say something even when \(f^{\prime} \) is not invertible, as long as we know that \(f\) is continuously differentiable. 
%something left
...
\begin{theorem}
    Let \(E\) be an open subset of \(\mathbb{R} ^n\) and let \(f : E \to \mathbb{R} ^n\) be a function which is continuously differentiable on \(E\). Suppose \(x_0 \in E\) is the point s.t. the linear map \(f^{\prime}(x_0) : \mathbb{R} ^n \to \mathbb{R} ^n \) is invertible, then there exists an open set \(U \subseteq E\) s.t. \(x_0 \in U\) and an open set \(V \subseteq \mathbb{R} ^n\) s.t. \(f(x_0) \in V\) so that \(f: U \to V\) is a bijection. In particular, there is a linear map \(f^{-1} : V \to U \). Furthermore, this inverse map is differentiable at \(f(x_0)\), and 
    \[
        \left( f^{-1} \right)^{\prime}  (f(x_0)) = \left( f^{\prime} (x_0) \right)^{-1}.
    \]              
\end{theorem}
...
\begin{proof}
    ...
    \] which shows \(g\) is differentiable at \(0\) with \(g^{\prime} (0) = 0\). Now let's analyze \(g(x)\). Since \(f\) is continuously differentiable, \(g\) is also continuously differentiable. 

    Thus, \(\exists r > 0\) s.t. 
    \[
        \left\vert \frac{\partial g_j}{\partial x_i}  \right\vert \le \frac{1}{2\sqrt{n^t}} \text{ for } x \in B(0, r)  
    \]     
    since \(\frac{\partial g_j(0)}{\partial x_i} = 0\) and hence
    \[
    ...
    \end{align*}    
    Now if \(x, y \in B(0, r)\), then \(x + t(y - x) \in B(0, r)\) if \(0 \le t \le 1\). Thus, 
    \[
    ...
    \]  
    Let \(\Phi (x) = x + g(x)\). By \autoref{lm: first lemma for inverse function theorem}, \(\Phi :B(0, r) \to \mathbb{R} ^n\) is one to one and \(B\left( 0, \frac{r}{2} \right) \subseteq \Phi \left( B(0, r) \right)  \). Since \(\Phi \) is one to one on \(B(0, r)\). Let \(W = \Phi ^{-1} \left( B\left(0, \frac{r}{2} \right)  \right) \), then \(\Phi : W \to B\left( 0, \frac{r}{2} \right) \) is one to one and onto. Now we know \(\Phi \) is continuous, so \(W\) is open (preimage of open set is open for continuous map). Also, \(0 \in W \) since \(\Phi (0) = 0 \in B\left( 0, \frac{r}{2} \right) \). Now we claim \(\Phi ^{-1} : B \left( 0, \frac{r}{2} \right) \to W \) is continuous. 
    Note that 
    \begin{align*}
    ...
    \]
    and implies \(\Phi ^{-1}\) is continuous. Thus, \(\Phi :W \to B\left( 0, \frac{r}{2} \right) \) is a homomorphism, i.e. \(\Phi \) is continuous and \(\Phi ^{-1}\) is also continuous. Since \(\Phi (0) = 0\), so \(\Phi ^{-1} (0) = 0\). Also, note that 
    \[
    ...
    Recall that \(\Phi : W \to B\left( 0, \frac{r}{2} \right) \) is a homomorpism. Now set 
    \[
    ...
    Now for \(x \in W\) we let \(y = f(x + x_0)\), so \(x + x_0 = f^{-1}(y)\).  Recall \(\Phi (x) = A^{-1} (f(x + x_0) - f(x_0)) = A^{-1} (y - f(x_0))\). Hence,
    \[
    ...
    This shows
    \begin{align*}
        \left( f^{-1} \right)^{\prime} (f(x_0)) &= \left( \Phi ^{-1} \right) \left( A^{-1} (f(x) - f(x_0)) \right) (A^{-1})^{\prime} (0) = (\Phi ^{-1})(0) \circ A^{-1} = IA^{-1} = (f^{\prime} (x_0))^{-1}   
    \end{align*}
