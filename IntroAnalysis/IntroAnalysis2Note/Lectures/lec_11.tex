...
\begin{proof}
    ...
    We know that 
    \[
        m^*(B_k) = m^*(B_k \cap E) + m^*(B_k \setminus E) \text{ by exercises.} 
    \]      
    Thus,
    \[
    ...
    Hence,
    \[
        m^*(A) + \varepsilon \ge \sum_{k=1}^{\infty} m^*(B_k) \ge m^*(A \cap E) + m^*(A \setminus E), 
    \]
    and we're done.
\end{proof}
...
\begin{proof}[proof of (a)]
    ...
    \[
        A \cap (\mathbb{R} ^n \setminus E) = A \setminus E \text{ and } A \setminus (\mathbb{E} \setminus E) = A \cap E. 
    \]   
    Thus,
    \[
...
\begin{proof}[proof of (e)]
    Note that every open boxes is the intersection of many half-spaces of the form 
    \[
...
\begin{proof}
    ...
    Now if we have pairwise disjoint \(E_1, E_2, \dots , E_k\), we can pick \(A = \varnothing \), and thus 
    \[
...
\begin{proof}
    ...
    \]  
    Hence, \(\bigcup_{k=1}^{\infty} \Omega _{j_k}\) is measurable. 
