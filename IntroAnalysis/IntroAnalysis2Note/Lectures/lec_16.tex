...
\begin{lemma}[Fatou's lemma] \label{lm: Fatou}
...
\begin{proposition}
    Let \(\Omega \) be a measurable subset of \(\mathbb{R} ^n\), and let \(f, g: \Omega \to \mathbb{R} ^*\) be absolutely integrable functions. Then the following statements hold. 
    \begin{itemize}
        \item [(a)] \(\forall c \in \mathbb{R}\), the function \(cf\) is absolutely integrable, and \(\int _\Omega cf = c \int _\Omega f\). 
        \item [(b)] The function \(f + g\) is absolutely integrable, and \(\int _\Omega (f + g) = \int _\Omega f + \int _\Omega g\).
...
\begin{proof}
    \hfill 
    \begin{itemize}
        \item [(a)] We first show \(cf\) is absolutely integrable. Since \(\vert cf \vert = \vert c \vert \vert f \vert   \), we have \(\int _\Omega \vert cf \vert = \vert c \vert  \int _\Omega \vert f \vert  \) since \(f\) is absolutely integrable and thus \(\int _\Omega \vert f \vert < \infty \). 
        
        Now it remains to compute its integral. We consider two cases. \\
        \textbf{Case 1:} \(c \ge 0\). In this case, \((cf)^+ = c f^+\) and \((cf)^- = c f^-\). Thus, 
        \begin{align*}
            \int _\Omega cf &= \int _\Omega (cf)^+ - \int _\Omega (cf)^- = \int _\Omega cf^+ - \int _\Omega cf^- = c \int _\Omega f^+ - c \int _\Omega f^- \\
            &= c \left( \int _\Omega f^+ - \int _\Omega f^- \right) = c \int _\Omega f. 
        \end{align*}    
        \textbf{Case 2:} \(c < 0\). Now we have \((cf)^+ = -c f^-\) and \((cf)^- = -c f^+\). Hence,
        \begin{align*}
            \int _\Omega cf &= \int _\Omega (cf)^+ - \int _\Omega (cf)^- = \int _\Omega (-c) f^- - \int _\Omega (-c) f^+ = (-c) \int _\Omega f^- - (-c) \int_\Omega f^+ \\
            &= c \int _\Omega f^+ - c \int _\Omega f^- = c \left( \int _\Omega f^+ - \int _\Omega f^- \right) = c \int _\Omega f. 
        \end{align*}    
        \item [(b)] We first show that \(f + g\) is absolutely integrable. Note that 
        \[
            \int _\Omega \vert f + g \vert \le \int _\Omega \vert f \vert + \vert g \vert = \int _\Omega \vert f \vert + \int _\Omega \vert g \vert < \infty    
        \]
        by the properties of Lebesgue integral on non-negative measurbale functions, and hence we're done. 
        ...
        \begin{align*}
            \int _\Omega (f + g) &= \int _\Omega (f^+ + g^+) - (f^- + g^-) \\
            &= \int _\Omega (f^+ + g^+) - \int _\Omega (f^- + g^-) = \left( \int _\Omega f^+ + \int _\Omega g^+ \right) - \left( \int _\Omega f^- + \int _\Omega g^- \right) \\ 
            &= \left( \int _\Omega f^+ - \int _\Omega f^- \right) + \left( \int _\Omega g^+ - \int _\Omega g^- \right) = \int _\Omega f + \int _\Omega g.     
        \end{align*} 
        \item [(c)] Assume that \(f(x) \le g(x)\) for all \(x \in \Omega\), then \(g - f\) is non-negative everywhere on \(\Omega\). By (a),(b), we know \(g - f = g + (-f)\) is absolutely integrable. Also,
        \[
            \int _\Omega g = \int _\Omega (f + (g - f)) = \int _\Omega f + \int _\Omega (g - f).
        \]
        Since \(g - f\) is non-negative and measurable, \(\int _\Omega (g - f) \ge 0\), and thus \(\int _\Omega g \ge \int _\Omega f\), and we're done.
        \item [(d)] Assume that \(f(x) = g(x)\) for almost every \(x \in \Omega\). Define \(h \coloneqq f - g\). Then \(h = 0\) almost everywhere. Also, since \(f, g\) are absolutely integrable, (a) and (b) shows that \(h\) is absolutely integrable. Because \(h = 0\) almost everywhere, we also have \(\vert h \vert = 0\) almost everywhere. This shows \(\int _\Omega \vert h \vert = 0 \) by properties of Lebesgue integral for non-negative measurable functions. Now apply the traingle inequality:
        \[
            \left\vert \int _\Omega h \right\vert \le \int _\Omega \vert h \vert = 0,  
        \]
        which shows \(\int _\Omega h = 0\). This shows 
        \[
            0 = \int _\Omega h = \int _\Omega f - g = \int _\Omega f - \int _\Omega g,
        \] 
        and thus \(\int _\Omega f = \int _\Omega g\). 
    \end{itemize}
\end{proof}