...
\begin{definition}
    ...
    \]  
    where \(b_i \ge a_i\) are real nnumbers. We define the volume \(\mathrm{vol} (B) \) of this box to be the number 
    \[
...
\begin{proof}[proof of (x)]
    ...
    \] 
    so we're done. Now we assume \(m^* (A_j) < \infty \) for \(j \in J\). Given \(\varepsilon > 0\), \(\exists \) open boxes \(B_{j, k}\) s.t. \(A_j \subseteq \bigcup_{k=1}^{\infty} B_{j, k} \) and 
    \[
...
\begin{proof}
    ...
    \begin{explanation}
        ...
        \[
            \left( U_j \right)_{j=1}^k \text{ s.t. } B \subseteq \bigcup_{j=1}^{k} U_j.    
        \]
        Thus,
        \[
            \sum_{j=1}^k \mathrm{vol} (U_j) \le \sum_{j \in J} \mathrm{vol} (B_j).    
        \]
        ...
        Thus,
        \[
            \mathrm{vol}(B) = \int _Q f(x) \, \mathrm{d} x \le \int \sum_{j=1}^k f_j (x) \, \mathrm{d} x = \sum_{j=1}^k \int f_j (x) \, \mathrm{d} x = \sum_{j=1}^k \mathrm{vol} (U_j) \le \sum_{j \in J} \mathrm{vol} (B_j) .     
        \]
    \end{explanation}
