...
\begin{proof}
    ...
    \[
        \lVert (x, Tx) \rVert_{X \times Y} = \lVert \pi_X^{-1} x \rVert_{X \times Y} \leq C \lVert x \rVert_X.
    \]
    Therefore \(\lVert Tx \rVert_Y \leq C \lVert x \rVert_X\) for every \(x \in X\). Thus \(T\) is bounded.
\end{proof}
