...
\begin{proof}
    ...
    \]        
    Hence \(f\) is continuous on the compact set \(S\). It therefore attains its minimum on \(S\). Thus there exists \(x_0 \in S\) s.t. \(\lVert x_0 \rVert = \min _{x \in S} \lVert x \rVert \). Since \(x_0 \in S\), we have \(x_0 \neq 0\). Hence, because \(\lVert \cdot \rVert \) is a norm, \(\lVert x_0 \rVert > 0 \). Set \(\delta \coloneqq \lVert x_0 \rVert > 0\). Then \(\lVert x \rVert \ge \delta \) for every \(x \in S\). Now let \(x \in X\) be arbitrary. If \(x = 0\), then 
    \[
...
\begin{proof}
    ...
    Since \(X\) is finite dimensional, all norms on \(X\) are equivalent. Therefore \(\lVert \cdot \rVert_A \) is equivalent to the given norm \(\lVert \cdot \rVert_X \). In particular, there exists a constant \(C \ge 1\) s.t. 
    \[
