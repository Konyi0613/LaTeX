...
Suppose \(f: \mathbb{R} ^n \to \mathbb{R} ^n\) has 
    \[
        ...
        \end{dcases}
    \]
then \(f\) is differentiable on \(E\) and \(f^{\prime} (x) = 0\) and \(f\) is not differentiable on \(\mathbb{R} ^n\) at \(0\). This is because on \(E\), \(f\) is a constant function, so 
\[
    \frac{\lVert f(x) - f(x_0) - L(x - x_0) \rVert }{\lVert x - x_0 \rVert } = 0
\]        
for \(x_0 \in E, x \in E \setminus \left\{ x_0 \right\} \). On \(\mathbb{R} ^n\) and \(x_0 = 0\), then for \(x \in \mathbb{R} ^n \setminus \left\{ x_0 \right\} = \mathbb{R} ^n \setminus \left\{ 0 \right\}  \), then if we approach to \(0\) along \(E\), then it gives 
\[
    \lim_{\substack{x \to x_0 \\ x \in E}}  \frac{\lVert f(x) - f(x_0) - L(x - x_0) \rVert }{\lVert x - x_0 \rVert } = 0,
\]      
but if we approach \(0\) along \(x_n < 0\), then it becomes 
\[
    \lim_{\substack{x \to x_0 \\ x \in \mathbb{R} ^n \setminus E}}  \frac{\lVert f(x) - f(x_0) - L(x - x_0) \rVert }{\lVert x - x_0 \rVert } = \lim_{\substack{x \to x_0 \\ x \in \mathbb{R} ^n \setminus E}} \frac{\lVert x - 0 - L(x - x_0) \rVert }{\lVert x \rVert } = 1
\]  
...
\begin{remark}
    Because of \autoref{lm: uniqueness of derivatives}, the derivative of \(f\) at interior points \(x_0\) is unique, and thus we may safely write it as \(f^{\prime} (x_0)\) or \(Df(x_0)\). Thus \(f^{\prime} (x_0)\) is the unique linear transformation \(f^{\prime}(x_0) : \mathbb{R} ^n \to \mathbb{R} ^m\) such that 
    \[
...
\begin{theorem} \label{thm: if f continuously differentiable then f differentiable}
    Let \(E \subseteq \mathbb{R} ^n\), let \(f : E \to \mathbb{R} ^m\), \(F \subseteq E\), and let \(x_0 \in \mathrm{Int}(F) \) (the partial derivative exists on a neighborhood of \(x_0\)). If all partial derivative \(\frac{\partial f}{\partial x_j} \) exists on \(F\) and are continuous at \(x_0\), then \(f\) is differentiable at \(x_0\), and 
    \[
...
\begin{proof}
    ...
    \]   
    By the chain rule (See \autoref{lm: chain rule}), 
    \[
    ...
    \begin{align*}
        &\left\lVert f \left( x^{(j)} \right) - f \left( x^{(j-1)} \right) - \frac{\partial f}{\partial x_j} (x_0) v_j    \right\rVert = \left\lVert \sum_{i=1}^m \left( f_i \left( x^{(j)} \right) - f_i \left( x^{(j-1)} \right) - \frac{\partial f_i}{\partial x_j} (x_0) v_j  \right) e_i   \right\rVert \\
        &\le \sum_{i=1}^m \left\vert f_i \left( x^{(j)} \right) - f_i \left( x^{(j-1)} \right) - \frac{\partial f_i}{\partial x_j} (x_0) v_j \right\vert \le m \frac{\varepsilon }{nm} \vert v_j \vert = \frac{\varepsilon}{n} \vert v_j \vert \le \frac{\varepsilon}{n} \lVert x - x_0 \rVert.     
    \end{align*}
