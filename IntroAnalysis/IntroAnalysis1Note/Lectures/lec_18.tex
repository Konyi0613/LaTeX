...
In particular, \(f\) is continuous at any boundary point where the series converges. 
 ...
 \begin{remark}
    Abel's theorem tells us if the boundary series \(\sum_{n=0}^{\infty} c_n R^n  \) or \(\sum_{n=0}^{\infty} c_n \left( -R \right)^n  \) converges, then the series is continuous at the boundary points, and the limiting value is exactly substituting \(a+R\) or \(a-R\) into the series. (note the definition of continuity).   
 \end{remark}
...
\begin{proof}[Proof of Abel's theorem]
    ...
    \begin{itemize}
        ...
        \]
        \item \textbf{Step 6:} Let \(y \to 1^-\). The finite sum \(\sum_{n=0}^N \left\vert S_{n+1} \right\vert \left( y^n - y^{n+1} \right) \to 0  \), and \(y^{N+1} \to 1\), which gives 
        \[
