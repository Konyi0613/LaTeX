...
\begin{remark}
    If \(f(x) = \sum_{n=0}^{\infty} c_n (x-a)^n \) is a formal power series, and \(R\) is the radius of convergence of \(f\), then the radius of convergence of 
    \[
        f^{\prime} (x) = \sum_{n=1}^{\infty} n c_n (x-a)^{n-1}, \quad \int _y^z f(x) \, \mathrm{d} x = \sum_{n=0}^{\infty} \frac{c_n (z-a)^{n+1}}{n+1} - \sum_{n=0}^{\infty} \frac{c_n (y-a)^{n+1}}{n+1}   
    \] are both \(R\) for \([y, z] \subseteq (a-R, a+R)\). 
\end{remark}