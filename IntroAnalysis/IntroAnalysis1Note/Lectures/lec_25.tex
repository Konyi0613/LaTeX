...
\begin{proof}
    ...
    \]
    so by Weierstrass \(M\)-test, we know \(\left\{ F_N \right\}_{N=1}^{\infty}   \) converges uniformly to a continuous function \(F \in C(\mathbb{R} / \mathbb{Z} , \mathbb{C} )\). Hence, we know 
    \[
...
\begin{proof}
    ...
    \[
        \left\lVert f - \sum_{n = -N}^N \hat{f} (n) e_n  \right\rVert < \varepsilon \text{ for all } N \ge N_9 
    \]  
    Now suppose 
    \[
...
\begin{eg}
    ...
    \[
        \lVert f \rVert_2^2 = \int _0^1 \left( x - \frac{1}{2} \right)^2 \, \mathrm{d} x = \frac{u^3}{3} \left.  \right]_{-\frac{1}{2}}^{\frac{1}{2}} = \frac{1}{12}.  
    \]    
