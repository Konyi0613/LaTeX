...
\begin{theorem}[Analysis I] \label{thm: double series absolutely conv then can exchange index}
    ...
    \[
        \sum_{m} \sum_{n} a_n a_m = \sum_{n} \sum_{m} a_m a_n.    
    \] 
\end{theorem}
...
\begin{proof}
    ...
    \begin{itemize}
        \item \textbf{Step 1:} Absolute convergence of \(f\) and \(g\). By our assumption \(f\) and \(g\) are analytic on \((a-r,a+r)\), so \(f(x) = \sum_{n=0}^{\infty} c_n (x-a)^n \) and \(g(x) = \sum_{n=0}^{\infty} d_n(x-a)^n \) convergs absolutely when \(\vert x-a \vert < r \). In particular, for each fixed \(x \in (a-r, a+r)\), the sum 
        \[
        ...
        \begin{align*}
            \sum_{n=0}^{\infty} d_n (x-a)^n \sum_{m=0}^{\infty} c_m (x-a)^m = \left( \sum_{n=0}^{\infty} d_n (x-a)^n  \right) f(x) = g(x) f(x).   
        \end{align*}
        Also, we have
        \begin{align*}
            f(x) g(x) &= \sum_{n=0}^{\infty} \sum_{m=0}^{\infty} c_m (x-a)^m d_n (x-n)^n \\
        ...
        \end{align*}  
        \item \textbf{Step 6:} Interchanging the order of summation. 
        \[
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \[
            \int _a^b \exp (x) \, \mathrm{d} x = \exp (b) - \exp (c). 
        \]
...
\begin{proof}
    ...
    \] so 
    \[
        \exp \left( \frac{p}{q} \right) = \left( \exp (p) \right)^{\frac{1}{q}} = \left( e^p \right)^{\frac{1}{q}} = e^{\frac{p}{q}}.   
