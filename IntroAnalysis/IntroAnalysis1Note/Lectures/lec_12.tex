...
\begin{remark}
    ...
    However, if \(x_0 \in E\), then by Terrence Tao's definition, \(\lim_{x \to x_0, x \in E} f(x_0) = L\) if \(d_Y \left( f(x), L \right) < \varepsilon  \) for all \(\varepsilon > 0\) and for \(x \in E\) s.t. \(d_X(x, x_0) < \delta \) for the corresponding \(\delta \). Also, if \(x_0 \notin E\), then since \(x_0 \in \overline{E} \), so \(\exists x \in E\) s.t. \(d(x, x_0) < \delta \), so the definition of \(\lim_{x \to x_0, x \in E} f(x)\) is well-defined. In our notation, other textbooks' definition is like
    \[
