...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \[
            \sup _{x \in X} d_Y\left( f^{(n)}(x), f(x) \right)  = d_\infty \left( f^{(n)}, f \right) < \varepsilon,
        \] so 
        \[
            d_Y \left( f^{(n)}(x), f(x) \right) \le \sup _{x \in X} d_Y\left( f^{(n)}(x), f(x) \right) < \varepsilon 
