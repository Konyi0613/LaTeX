...
\section{Composition of real analytic functions is analytic}
\begin{theorem}
    Suppose \(f, g : \mathbb{R} \to \mathbb{R} \) are both real analytic, then \(g(f(x))\) is also real analytic.  
\end{theorem}
\begin{proof}
...
\[
f(x) = \sum_{m \ge 0} a_m (x - c)^m, 
\qquad |x - c| \le R, 
\qquad \sum_{m \ge 0} |a_m|R^m < \infty,
\]

\[
g(y) = \sum_{n \ge 0} b_n (y - f(c))^n,
\qquad |y - f(c)| \le S,
\qquad \sum_{n \ge 0} |b_n|S^n < \infty.
\]

Since \(f\) is continuous (meaning \(f(x) \to f(c) = a_0\) when \(x \to c\)), there is some \(r \le R\) such that \(\lvert f(x) - f(c)\rvert \le S\) whenever \(\lvert x - c\rvert \le r\).  Thus both power series are absolutely convergent for \(\lvert x - c\rvert \le r\), and in particular:

\[
g(f(x)) = \sum_{n \ge 0} b_n \Bigl( \sum_{m \ge 1} a_m (x - c)^m \Bigr)^n.
\]

However, to validly rearrange terms we need a stronger bound.  Notice:

\[
\sum_{m \ge 1} |a_m| |x - c|^m
\;\le\;
|x - c| \sum_{m \ge 1} |a_m| R^{m-1}
\;=\; M\,|x - c|,
\]
...
\[
M = \frac1R \sum_{m \ge 1} |a_m| R^m \;\le\;\frac1R \sum_{m \ge 0} |a_m| R^m < \infty.
\]

Take \(r = \min\{S / M,\, R\}\).  Then for \(\lvert x - c\rvert \le r\):

\[
\sum_{m \ge 1} |a_m| |x - c|^m
\;\le\; M |x - c| \;\le\; M r \;\le\; S.
\]

So:

\[
\sum_{n \ge 0} |b_n| \Bigl( \sum_{m \ge 1} |a_m| |x - c|^m \Bigr)^n
\;\le\;
\sum_{n \ge 0} |b_n| S^n
< \infty.
\]

Hence the nested sums converge absolutely, and we may rearrange terms freely.  We can expand

\[
g(f(x)) = \sum_{n \ge 0} b_n (x - c)^n \Bigl( \sum_{m \ge 1} a_m (x - c)^{m-1} \Bigr)^n
\]

\[
= b_0 + b_1 (x - c) \bigl(a_1 + a_2(x - c) + a_3(x - c)^2 + \cdots \bigr)
\]

\[
\quad + b_2 (x - c)^2 \bigl(a_1 + a_2 (x - c) + \cdots \bigr)^2
\;+\; b_3 (x - c)^3 \bigl(a_1 + a_2 (x - c) + \cdots \bigr)^3 + \cdots
\]

Collecting like powers of \((x-c)\), one obtains

\[
g(f(x)) = \sum_{p \ge 0} (x - c)^p 
\sum_{\substack{n \ge 0 \\ n \le p}} b_n
\sum_{\substack{k_1, k_2, k_3, \dots \ge 0 \\ k_1 + 2k_2 + 3k_3 + \cdots = p \\ k_1 + k_2 + k_3 + \cdots = n}}
\frac{n!}{k_1!\,k_2!\,k_3!\cdots} 
a_1^{k_1} a_2^{k_2} a_3^{k_3} \cdots.
\]

This shows \(g(f(x))\) has a convergent power series in \((x-c)\), so it is analytic at \(c\).
...
\begin{problem}[HW11 Q1]
    Let \(f \in C(\mathbb{R} / \mathbb{Z} ), \mathbb{C} \), and let \(\left( f_n \right)_{n=1}^{\infty}  \) be a sequence of function in \(C(\mathbb{R} / \mathbb{Z} , \mathbb{C} )\). 
    \begin{itemize}
        \item [(a)] Show that if \(f_n \to f\) uniformly, then \(f_n \to f\) in \(L^2\) metric. 
        \item [(b)] Show that if \(f_n \to f\) in \(L^2\)-metric and \(f_n \to f\) pointwise, then  \(f_n\) may not converge to \(f\) uniformly. 
        \item [(c)] Show that if \(f_n \to f\) in \(L^2\)-metric, then \(f_n\) may not converge to \(f\) pointwise. 
        \item [(d)] Show that if \(f_n \to f\) pointwise, then \(f_n\) may not converge to \(f\) under \(L^2\) metric.                
    \end{itemize}   
\end{problem}

\begin{problem}[HW11 Q2]
    Let \(\left\{ \phi _N \right\} \subseteq C(\mathbb{R} /\mathbb{Z} , \mathbb{C} ) \) satisfy
    \[
        \int _0^1 \phi _N (t) \, \mathrm{d} t = 1  \text{ and }  \int _0^1 \left\vert \phi _N(t) \right\vert \, \mathrm{d} t \le M < \infty  \quad \forall  N \in \mathbb{N}  
    \]
    and 
    \[
        \lim_{N \to \infty} \int _{\delta }^{1 - \delta } \left\vert \phi _N(t) \right\vert \, \mathrm{d} t = 0 \quad \forall 0 < \delta < 1.  
    \]    
    Suppose \(f \in C(\mathbb{R} / \mathbb{Z} , \mathbb{R} )\). Prove that 
    \[
        \lim_{N \to \infty} \int _0^1 f(x - t) \phi _N(t) \, \mathrm{d} t = f(x) \text{ uniformly for } x \in \mathbb{R} .  
    \] 
\end{problem}
\begin{proof}
    Let 
    \[
        I_N(x) = \int _0^1 f(x - t) \phi _N(t) \, \mathrm{d} t. 
    \]
    WTS: \(\lim_{N \to \infty} \sup _{x \in \mathbb{R} } \left\vert I_N(x) - f(x) \right\vert = 0  \). Since \(f\) is continuous and \(1\)-periodic, so \(f\) is uniformly continuous on \(\mathbb{R} \). Thus, for all \(\varepsilon > 0\), there exists \(\delta > 0\) s.t. 
    \[
        \forall \vert x - y \vert < \delta \implies \left\vert f(x) - f(y) \right\vert < \frac{\varepsilon }{2M},  
    \]      
    and WLOG set \(\delta < \frac{1}{2}\). Now if we split 
    \[
        I_N(x) - f(x) = \underbrace{\int _0^\delta \left( f(x - t) - f(x) \right) \phi _N(t) \, \mathrm{d} t}_{A_N(x)} + \underbrace{\int _\delta ^{1 - \delta } \left( f(x - t) - f(x) \right) \phi _N(t) \, \mathrm{d} t}_{B_N(x)} + \underbrace{\int _{1 - \delta }^1 \left( f(x - t) - f(x) \right) \phi _N(t) \, \mathrm{d} t}_{C_N(x)}.
    \] 
    Note that 
    \begin{align*}
        \left\vert A_N(x) \right\vert + \left\vert C_N(x) \right\vert &\le \int _0^\delta \left\vert f(x - t) - f(x) \right\vert  \left\vert \phi _N(t) \right\vert  \, \mathrm{d} t + \int _{1 - \delta }^1 \left\vert f(x - t) - f(x) \right\vert  \left\vert \phi _N(t) \right\vert  \, \mathrm{d} t \\
        &\le \int _0^{\delta } \frac{\varepsilon}{2M} \left\vert \phi _N(t) \right\vert \, \mathrm{d} t + \int _{1 - \delta }^{1} \frac{\varepsilon}{2M} \left\vert \phi _N(t) \right\vert \, \mathrm{d} t \\
        &\le \int _0^1 \frac{\varepsilon}{2M} \left\vert \phi _N(t) \right\vert \, \mathrm{d} t \le \frac{\varepsilon}{2M} \le M = \frac{\varepsilon}{2}.     
    \end{align*}
    Also, 
    \begin{align*}
        \vert B_N(x) \vert &\le \int _\delta ^{1 - \delta } \left\vert f(x - t) - f(x) \right\vert \vert \phi _N(t) \vert \, \mathrm{d} t \le 2S \int _{\delta }^{1 - \delta } \vert \phi _N(t) \vert \, \mathrm{d} t \to 0 
    \end{align*}
    if we let \(\vert f \vert \le S \). (Note that \(f \in C(\mathbb{R} / \mathbb{Z} , \mathbb{C} )\) so \(f\) is bounded)
    Hence, 
    \[
        \sup _{x \in \mathbb{R} } \vert I_N(x) - f(x) \vert \le \vert A_N(x)  \vert + \vert B_N(x) \vert  + \vert C_N(x) \vert \le \varepsilon  
    \]  
    for large enough \(N\). 
\end{proof}

\begin{problem}[HW11 Q6]
    Let \(f, g \in C(\mathbb{R} / \mathbb{Z} , \mathbb{R} )\). Define their periodic convolution \(f * g: \mathbb{R} \to \mathbb{R} \) by
    \[
        (f * g)(x) = \int _0^1 f(y) g(x - y) \, \mathrm{d} y. 
    \]  
    Prove that if \(f\) is smooth, then \((f * g)\) is smooth.  
\end{problem}
...
\chapter{Supplement}
\section{\(L^2\) norm metric on \(C(\mathbb{R} / \mathbb{Z} , \mathbb{C} )\) is not complete} \label{sec: L2 norm metric}
\includepdf[pages=-]{./Supplement/p20-p23.pdf}