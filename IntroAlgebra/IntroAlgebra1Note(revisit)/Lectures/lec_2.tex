...
\begin{proof}
	...
	Conversely, we show that \(d\) is the \emph{greatest} common divisor, and for this we
	use the other half of the information we have about \(d\): that it is a
	\(\mathbb Z\)-linear combination of \(a\) and \(b\). Let \(c\) be any common divisor
	of \(a\) and \(b\), say \(a=ca'\) and \(b=cb'\) with \(a',b'\in\mathbb Z\). Then
	\[
		d=ax_0+by_0=ca'x_0+cb'y_0=c\,(a'x_0+b'y_0),
	\]
	and \(a'x_0+b'y_0\in\mathbb Z\), so \(c\mid d\). Thus every common divisor of \(a\)
	and \(b\) divides \(d\).

	Finally, \(c\mid d\) together with \(d>0\) forces \(\abs{c}\leq d\), in particular
	\(c\leq d\). So \(d\) is a common divisor of \(a\) and \(b\) that is at least as
	large as every other one, which is exactly to say \(d=\gcd(a,b)\).
\end{proof}
