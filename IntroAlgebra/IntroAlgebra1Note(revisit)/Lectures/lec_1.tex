...
Write \(P \cdot Q = ac + bd\) for the dot product. Then
\[
	(P-Q)\cdot(P-Q)
	...
	= \norm{P-Q}^2,
\]
from which it follows that every isometry preserves dot products:
\(\varphi(P)\cdot\varphi(Q) = P\cdot Q\). Conversely, a map preserving dot products is an
isometry. Since \(P \cdot Q = \norm{P}\norm{Q}\cos\theta\), every isometry also preserves
\emph{angles}; in particular \(P\) and \(Q\) are orthogonal if and only if
\(P\cdot Q = 0\), so isometries preserve perpendicularity.
...
\begin{proof}
	...
	\]
	contradicting that \(\varphi\) preserves distance. Conversely, if \(P, Q, R\) are not
	collinear they are the vertices of a triangle; if their images were collinear, the strict
	inequality above becomes an equality, again a contradiction.
\end{proof}
...
\begin{corollary}\label{cor:isometry-determined}
	If \(O, P, Q\) are noncollinear points and \(\varphi, \psi\) are isometries of the plane
	with \(\varphi(P) = \psi(P)\) and \(\varphi(Q) = \psi(Q)\), then \(\varphi = \psi\).
\end{corollary}

\begin{proof}
	Since \(O, P, Q\) are noncollinear, the list \(P, Q\) is linearly independent in
	\(\mathbb{R}^2\); as \(\dim(\mathbb{R}^2) = 2\), it is a basis, and two linear
