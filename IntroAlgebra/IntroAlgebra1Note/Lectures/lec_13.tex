...
\begin{proposition}
    Let \(G\) be a group of order \(pq\) with \(p,q\) distinct and both prime s.t. \(q \not\equiv 1 \mod{p}\), then 
    \[
        G \simeq \mathbb{Z} / pq \mathbb{Z} .
    \] i.e. The group of order \(pq\) is unique.  
\end{proposition}
\begin{proof}
    Since \(\vert G \vert = pq \), we know \(n_q \equiv 1 \mod{q}\) and \(n_q \mid p\), so \(n_q = 1, p\). Note that \(n_q = 1\). Thus, we know Sylow \(q\)-subgroup is a unique \(Q\), and it is normal. Similarly we can show \(n_p = 1\) and thus Sylow \(p\)-subgroup is a normal \(P\). Hence, \(\vert P \vert = p \) and \(\left\vert Q \right\vert = q \), and since \(P \cap Q = \left\{ 1 \right\}  \), so we have 
    \[
        P \times Q \simeq PQ=G.
    \]  This proves \(G \simeq \mathbb{Z} / p \mathbb{Z} \times \mathbb{Z} / q\mathbb{Z} \) and since \(p, q\) are distinct prime, so 
    \[
