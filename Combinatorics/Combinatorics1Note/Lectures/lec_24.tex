...
\begin{explanation}
    ...
    \begin{itemize}
        ...
        \[
            \mu _P(x, y) = 0 \sum_{x \le z < y} \mu _P(x,z) = \begin{dcases}
                -1, &\text{ if } y = x + 1 ;\\
...
\end{eg}
\begin{explanation}
    If \(S \subsetneq T\), then \(\mu _P(S, T) = 0\). Then we can do induction on \(\vert T \setminus S \vert \). 
    \begin{itemize}
...
\begin{proof}
    ...
    \end{align*}
    Thus, \(\hat{g} = \hat{f} * \zeta _P\), and thus 
    \begin{align*}
    ...
    Thus, 
    \begin{align*}
        f(y) &= \widetilde{f} (x_0, y) = (\widetilde{g} * \mu _P)(x_0, y) = \sum_{x \in [x_0, y]} \widetilde{g} (x_0, x) \mu _P(x, y) = \sum_{x \in [x_0, y]} g(x) \mu _P(x, y). 
    \end{align*}
\end{proof}
...
\subsection{Application 1} For any \(n\), 
\[
...
\subsection{Application 2: Inclusion-Exclusion}
