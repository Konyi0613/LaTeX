...
\begin{proof}[proof of Mirsky]
    ...
    \begin{itemize}
        ...
        \begin{remark}
            If \(h(P^{\prime} ) < h(P) - 1\), then by induction hypothesis there exists an antichain decomposition:
            \[
                A_1 \cupdot A_2 \cupdot \dots \cupdot A_{h(P^{\prime} )},
            \] 
            and we can seperate some antichain into \(2\) parts or more parts to make an antichain decomposition of size \(h(P) - 1\).  
        \end{remark}  
    \end{itemize} 
\end{proof}

\begin{proof}[proof of Dilworth]
    We prove by induction on \(\vert P \vert \). 
    \begin{itemize}
        \item Base case: \(\vert P \vert = 1\). In this case, \(P\) is itself a chain, so \(P = P\) is the smallest chain partition, and we're done. 
        \item Induction step: \((\vert P \vert  \ge 2)\) If there exists a chain \(C\) that intersecting every maximum antichain, then we can proceed as before:
        \begin{itemize}
        ...
        \begin{remark}
            Each \(C_i^+\) can contain only one element in it, and \(C_i^-\) has same property, so there exists a chain partition of \(P^+\) and \(P^-\), of size \(w\) by induction.     
        \end{remark}
    \end{itemize} 
\end{proof}
...
\begin{explanation}
    ...
    Hence, 
    \[
        \sum_{T \subseteq S} g(T) (-1)^{\vert S \setminus T \vert } = f(S). 
    \]
\end{explanation}