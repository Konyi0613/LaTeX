...
\begin{itemize}
    ...
    \item Output: Either an \(M\)-augmenting path or a vertex cover \(C \subseteq X \cup Y\) with \(\vert C \vert = \vert M \vert  \).     
\end{itemize}
...
\begin{algorithm}[H]
    ...
    \While(){True} {
        \If(){\(S = Q\)}{
            \(M\) is a maximum matching, which covers \(T \cup (X \setminus S)\), so   Done.
        } \Else() {
...
\end{theorem}
\begin{proof}
    We prove this algorithmically: given a weighting \(w\), we will find a perfect matching \(M\) and a cover \(c\) s.t. \(w(M) = \mathrm{val}(c) \), and \(M\) is a max weight matching and \(c\) is a min-value cover. 

    The idea is: Start with a cover \(c\). Build the equality subgraph \(G \subseteq K_{n, n}\):
    \[
        E(G) = \left\{ \left\{ x, y \right\} \in E(K_{n, n}): w(\left\{ x, y \right\} ) = c(x) + c(y)  \right\}. 
    \] 
    If \(G\) has a perfect matching \(M\) (use MMA), then we have a proposition: \(M\) and \(c\) are optimal. If not, use the cover from MMA to improve \(c\).          
\end{proof}