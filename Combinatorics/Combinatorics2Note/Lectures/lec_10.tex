...
Suppose, by contradiction, that \(M\) does not saturate all vertices in \(X\).  
Let
\[
...
\begin{theorem}[Duality for max weight matching]
    For any weighting \(w\) of \(E(K_{n, n})\), 
    \[
        \max \left\{ w(M) : M \text{ is a perfect matching}  \right\} = \min \left\{ \mathrm{val}(c): c \text{ is a weighted cover of } w   \right\}.  
    \] 
\end{theorem}
\begin{proof}
...
\( u : X \to \mathbb{R} \), \( v : Y \to \mathbb{R} \), satisfying \(u(x) + v(y) \ge w(x,y), \quad \forall (x,y) \in E.\) For example, we can choose \(u(x) = \max _{y \in Y} w(x, y)\) \(\forall x \in X\) and \(v(y) = 0\) \(\forall y \in Y\).    
