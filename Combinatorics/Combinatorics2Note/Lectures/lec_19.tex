...
\begin{definition}[Block]
    A block in a graph \(G\) is a maximal (under inclusion) induced connected subgraph with no cut-vertex, i.e. if the induced connected subgraph \(H \subseteq G\) is a block, then remove any \(z \in V(H)\) will not disconnect \(H\).  
\end{definition}
