...
First, sorting all edges by weight takes \(O(m \log _2 m)\) steps. Then, iterate through all eedges, if \(c_u = c_v\), do nothing, and this steps take \(O(1)\) steps. If not, then add edges and update components, which takes \(O(n)\) steps, and will run this case \(n - 1\) times. Thus, the total time:
\[
    O(m \log _2 m) + O(m) + O\left( n^2 \right) = O(m \log _2 m + n^2) = \begin{dcases}
        O(m \log m), &\text{ if } m = \Omega \left( \frac{n^2}{\log n} \right)  ;\\
        O\left( n^2 \right) , &\text{ if } O\left( \frac{n^2}{\log n} \right).
    \end{dcases} 
\]    
...
\begin{algorithm}[H]
    ...
    \(W \gets \left\{ u \right\} \), \(t(u) = 0\), \(\text{prev}(u) = \varnothing\). 

    For any \(x \in V \setminus \left\{ u \right\} \), \(t(x) = w(\left\{ u, x \right\} )\), \(\text{prev}(x) = u \). 
...
\begin{proof}
    ...
    \begin{explanation}
        ...
        \begin{itemize}
            ...
            Now we show (b). If we didn't update \(t(x), \text{prev}(x) \) in the \(i\)-th iteration, then this is true by induction. If we did update, then \(\text{prev}(x) = v_0 \) where \(v_0\) is the vertex added to \(W\) in the \(i\)-th iteration, and \(t(x) = t(v_0) + w(\left\{ v_0, x \right\} ) =\) weight of path \(u \to \dots \to v_0 \to x\), and \(u\) to \(\text{prev}(v_0) \) all in \(W\) by induction, and \(v_0\) was added to \(W\) in the \(i\)-th iteration. 
            
            Now we show (c). Suppose not, then \(\exists z \in V\setminus W\) minimizing \(t(z)\) but \(t(z) > d(z, u)\). Consider the cheapest path from \(z\) to \(u\).                  
        \end{itemize}
    \end{explanation}
