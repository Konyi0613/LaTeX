...
If we modify \autoref{alg: find component} so that we take the oldest element in \(Q\), then we call this algorithm \textbf{Breadth-first search}. 
...
\begin{eg}
    If \(G\) is an \(n\)-vertex graph (not necessarily complete). Define 
    \[
        w(e) = \begin{dcases}
            0, &\text{ if } e \in E(G) ;\\
            1, &\text{ if } e \notin E(G) .
