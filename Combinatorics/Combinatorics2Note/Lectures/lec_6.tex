\chapter{Matchings}
\lecture{6}{17 March.}{}
\section{Hall and Tutte and Dirac}
\begin{eg}
...
\begin{proof}[proof (Lovasz, 1975)]
    ...
    \begin{itemize}
        ...
        \item Case 2: There is a non-clique component in \(G - U\). We can find the following configuration:
        \begin{figure}[H]
            \centering
...
\begin{remark}
    Hence, in fact, this is an if and only if statement: \(o(G - S) \le \vert S \vert \) for every \(S \subseteq V(G)\) if and only if \(G\) has a perfect matching.  
\end{remark}