...
\begin{proof}[proof (Lovasz, 1975)]
    Suppose for contradiction. \(G\) is a counterexample on \(n\) vertices with the maximum number of edges. 
    \begin{claim}
    ...
    \begin{itemize}
        \item Case 1: Every component of \(G - U\) is a clique(complete subgraph). Each even clique admits a perfect matching. Each odd clique has a matching with only one unsaturated vertex. By Tutte's condition, number of odd cliques \(\le \vert U \vert \). Thus, we can match the unsaturated vertices to different vertices in \(U\). There will be even number of unused vertices in \(U\) (taking \(S = \varnothing \) in Tutte's condition, so \(G\) is even), and thus we can match them arbitrarily.  
        \item Case 2: There is a non-clique component in \(G \setminus U\). We can find the following configuration: %fill
        
        Let \(M_1\) be a perfect matching in \(G + \left\{ w, y \right\} \) and let \(M_2\) be a perfect matching in \(G + \left\{ x, z \right\} \). Let \(G^{\prime} = M_1 \cup M_2\). Then the components of \(G^{\prime} \) are either isolated edges (really in \(G\)) %fill 
        or even edges, alternating \(M_1\) and \(M_2\) edges. To build a matching \(M\) of \(G\), we can take all edges that are components in \(G^{\prime}\). Consider the cycle component \(C\) in \(G^{\prime} \). If the edges \(\left\{ y, w \right\} \) is not in \(C\), then take the \(M_1\) edges. If the edge \(\left\{ x, z \right\} \) is not in \(C\), then take the \(M_2\) edges. If both edges are in \(C\):                   
    \end{itemize}
\end{proof}