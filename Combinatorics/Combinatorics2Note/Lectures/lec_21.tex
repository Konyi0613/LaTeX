...
\begin{proof}
    ...
    We can embed\footnote{HW} \(G \subseteq G^{\prime}\), a \(\Delta \)-regular graph. (draw some edges on \(G\) to make it become \(G^{\prime}\)) Then, \(G^{\prime} \) is \(\Delta \)-edge-colorable, so restricting it to \(G\) gives a \(\Delta \)-edge-coloring, too.       
\end{proof}
...
By Vizing's, we can categorize all graphs into 2 types:
...
\begin{eg}
    All bipartite graphs are Type 1 by Konig's. Even cliques are Type 1. Odd cycles and odd cliques are Type 2.
\end{eg}

Now we prove Vizing's theorem.
