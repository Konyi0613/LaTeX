...
\begin{proof}
    Suppose \(\kappa (u, v) = k\) and \(\lambda (u, v) = \lambda \), then note that \(k \ge \lambda\) since we have to remove at least one vertex from each internally disjoint \(uv\)-path to disconnect \(u\) and \(v\). Now suppose we have \(\lambda \) internally disjoint \(uv\)-paths, say \(P_1, \dots , P_{\lambda}\), then any other \(uv\)-path must intersect with one of \(P_i\) at some edge. Now suppose the previous vertex of \(v\) in \(P_i\) is \(w_i\) (note that \(w_i \neq u\) since \(\left\{ u, v \right\} \notin E \)), then note that in \(G - \left\{ w_1, \dots , w_\lambda  \right\} \) \(u\) and \(v\) are disconnected. Thus, \(k \le \lambda \), and we're done.           
\end{proof}
