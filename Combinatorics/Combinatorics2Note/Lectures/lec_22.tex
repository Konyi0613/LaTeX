...
\begin{proof}
    ...
    Let \(G^{\prime} = G - \left\{ u, v \right\} - \varphi ^{-1}(i) \), i.e. we remove all \(i\)-colored edges, as well as \(\left\{ u, v \right\} \). Left with a \((k - 1)\)-edge-coloring of \(G^{\prime} - v\), and \(d_{G^{\prime}} (v) = k - 1\), \(d_{G^{\prime} } (u) = d_G(u) - 1\) for all \(u \in N(v)\). By induction, \((G^{\prime} - v) \) is \((k - 1)\)-edge-colorable implies \(G^{\prime} \) can be \((k - 1)\)-edge-colorable. Now add back the edges in \(G - G^{\prime}\), all of the same color, then we get a proper \(k\)-edge-coloring of \(G\). 
...
\begin{proof}
    ...
    \end{itemize}
    Need to assign each vertex a color from its list. Equivalently, decide for each color, which vertices receive that color. 

    Now let \(c \in \bigcup_{v} L_v \) be a color, and let 
    \[
        S_c = \left\{ v \in V(\vv{D}): c \in L_v \right\} = \text{ all vertices that have } c \text{ on their list, and have not yet been colored}.  
    \]  
    Also, let \(I_c \subseteq S_c\) be a kernel of \(\vv{D} [S_c]\). Assign \(c\) to every vertex in \(I_c\). Since \(I_c\) is independent, such coloring is proper. We do this to every color \(c\), and after every round, we remove colored vertices from the graph, then we can prove that every vertex will finally be colored.
