...
\begin{remark}
    The minimum degree condition is best possible. Otherwise, consider two subgraphs, one of \(\frac{n}{2} - 1\) vertices, one contains \(\frac{n}{2} + 1\) vertices, where \(n\) is even, and there is an edge between every vertex in one subgraph to any vertex in the other subgraph. In this case, if there is a Hamilton cycle, then it can start from the subgraph of \(\frac{n}{2} + 1\) vertices, but whenever we start from a vertex from this subgraph to the other one, we need a vertex in the other subgraph to match with the starting point, but \(\frac{n}{2} - 1 < \frac{n}{2} + 1\), so we may not go back.    
\end{remark}
