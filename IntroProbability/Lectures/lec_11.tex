...
\begin{tikzpicture}[
    ...
    % top annotation
    \node[note] at (3.2,1.9) {on average $\dfrac{1}{\lambda}$ event};
    ...
    \draw[guide,decorate,decoration={brace,mirror,amplitude=5pt}] (0,-0.1) -- (1,-0.1)
        node[midway,below=0pt,ink] {$\dfrac{1}{\lambda}$ unit time interval};
...
\begin{explanation}
    ...
    Now 
    \[
        \mathbb{P} (E_i \mid E_j) = \begin{dcases}
            \mathbb{P} (E_i), &\text{ if } \vert i - j \vert > k + 1 ;\\
            0, &\text{ if } \vert i - j \vert \le k.
        \end{dcases}
    \]
    Note that when \(k\) is big, \(\mathbb{P} (E_i) \thickapprox 0\), so under this setting we have \((E_i^{\prime} )\) weakly independent. Now since \(\mathbb{P} (E_i)\) is small for all \(i\), Poisson paradigm applies. Now let 
    \[
        \lambda = \sum_{i=1}^{n-k+1} \mathbb{P} (E_i) = (n - k + 2) 2^{-(k + 1)}. 
    \]    
...
\begin{definition}[Geometric Distribution]
    \(X \sim \mathrm{Geom}(p) \) where \(0 \le p \le 1\) when independent trials with success probability \(p\) and we eant to count the number of trials until first success (inclusive). Hence, \(\mathrm{Geom}(p) \) takes values \(1, 2, 3, 4, \dots \).     
\end{definition}
