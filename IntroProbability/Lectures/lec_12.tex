...
An alternative way to compute \(\mathbb{E} [X]\): If \(X \sim \mathrm{NB}(r, p) \), then we can write \(X = X_1 + X_2 + \dots + X_r\), where \(X_r = \#\) of trials to go fromm \((i-1)\)-th success to \(i\)-th success, where each \(X_i \sim \mathrm{Geom}(p) \) is independent. 
