...
\end{lemma}
\begin{proof}
    We can prove it by the following graph.
    \begin{center}
\begin{tikzpicture}[x=1.25cm,y=1.25cm, >=Latex]
    % Axes
    \draw[->, thick] (0,0) -- (7.2,0) node[right] {$X$};
    \draw[->, thick] (0,0) -- (0,5.8) node[above] {$Y$};

    % Key coordinates
    \coordinate (A) at (2.2,1.8);
    \coordinate (B) at (6.1,4.7);

    % Main lower-left CDF region
    \fill[blue!10] (0,0) rectangle (B);

    % Removed strips
    \fill[red!18] (0,0) rectangle (2.2,4.7);
    \fill[red!18] (0,0) rectangle (6.1,1.8);

    % Added-back corner
    \fill[green!25] (0,0) rectangle (A);

    % Target rectangle
    \fill[pattern=north east lines, pattern color=black!70] (A) rectangle (B);
    \draw[very thick] (A) rectangle (B);

    % Guide lines
    \draw[dashed] (2.2,0) -- (2.2,5.0);
    \draw[dashed] (6.1,0) -- (6.1,5.0);
    \draw[dashed] (0,1.8) -- (6.4,1.8);
    \draw[dashed] (0,4.7) -- (6.4,4.7);

    % Axis labels
    \node[below] at (2.2,0) {$a_1$};
    \node[below] at (6.1,0) {$b_1$};
    \node[left] at (0,1.8) {$a_2$};
    \node[left] at (0,4.7) {$b_2$};

    % Region labels
    \node[align=center, text width=1.7cm] at (1.1,3.25)
        {$-F_{X,Y}$\\$(a_1,b_2)$};
    \node[align=center, text width=2.4cm] at (4.15,0.9)
        {$-F_{X,Y}(b_1,a_2)$};
    \node[align=center, text width=1.7cm] at (1.1,0.9)
        {$+F_{X,Y}$\\$(a_1,a_2)$};
    \node[align=center, text width=3.0cm, fill=white, inner sep=2pt] at (4.15,3.25)
        {$\mathbb{P}\bigl(a_1 \le X \le b_1,$\\$a_2 \le Y \le b_2\bigr)$};
\end{tikzpicture}
\end{center}
...
\begin{explanation}
    \hfill 
    \begin{itemize}
        \item [(a)] What is \(\mathbb{P} (X > Y)\)? Let \(C = \left\{ (x, y): X > Y \right\} \), then 
        \begin{align*}
            \mathbb{P} (X > Y) &= \mathbb{P} ((x, y) \in C) = \iint_C f_{X, Y}(u, v) \, \mathrm{d} u \, \mathrm{d} v = \int _{-\infty }^{\infty } \int _{-\infty }^u f_{X, Y} (u, v) \, \mathrm{d} v \, \mathrm{d} u \\
            &= \int _0^{\infty } \int _0^u e^{-(u+v)} \, \mathrm{d} v \, \mathrm{d} u = \int _0^{\infty } e^{-u} \int _0^u e^{-v} \, \mathrm{d} v \, \mathrm{d} u \\
            &= \int _0^{\infty } e^{-u} \left( \left[ -e^{-v} \right]_0^u  \right) \, \mathrm{d} u = \int _0^{\infty} e^{-u} \left( 1 - e^{-u} \right) \, \mathrm{d} u \\
            &= \int _0^{\infty} e^{-u} - e^{-2 u} \, \mathrm{d} u = \frac{1}{2}.          
        \end{align*}
        \item [(b)] What is \(\mathbb{P} (X \le t)\) for \(t \in \mathbb{R}\)? 
        \begin{align*}
            \mathbb{P} (X \le t) &= \iint_{\left\{ u \le t \right\} } f_{X, Y}(u, v) \, \mathrm{d} u \, \mathrm{d} v = \int _{-\infty }^{\infty} \int _{-\infty }^t f_{X, Y}(u, v) \, \mathrm{d} u \, \mathrm{d} v \\
            &= \int _0^{\infty} \int _0^t e^{-(u + v)} \, \mathrm{d} u \, \mathrm{d} v = 1 - e^{-t}.    
        \end{align*}
        \item [(c)] What is \(F_{\frac{X}{Y}} (t)\)? 
        \begin{align*}
            F_{\frac{X}{Y}}(t) &= \mathbb{P} \left( \frac{X}{Y} \le t \right) = \mathbb{P} (X \le tY) = \iint_{\left\{ u \le tv \right\} } f_{X, Y} (u, v) \, \mathrm{d} u \, \mathrm{d} v \\
            &= \int_{-\infty }^{\infty} \int _{-\infty }^{tv} f_{X, Y}(u, v) \, \mathrm{d} u \, \mathrm{d} v = \int_{0}^{\infty} \int _{0}^{tv} e^{-(u + v)} \, \mathrm{d} u \, \mathrm{d} v = 1 - \frac{1}{t+1}.    
        \end{align*}
    \end{itemize}
\end{explanation}