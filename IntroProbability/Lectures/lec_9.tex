...
\begin{explanation}
        \begin{align*}
            \mathbb{E} [X] &= \sum_{x_i} x_i \mathbb{P} (X = x_i) = \sum_{x_i} \mathbb{P} (\left\{ s : X(s) = x_i \right\} ) \\
...
\begin{definition}
    Random variables \(X_1, X_2, \dots \) are independent if for any \(x_1, x_2, \dots \), then events 
    \[
