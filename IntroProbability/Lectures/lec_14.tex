...
\begin{proof}
    ...
    \begin{remark}
        This proof only holds for positive real numbers, otherwise we cannot use technique like \(g(\underbrace{1 + 1 + \dots + 1}_{r}) = g(1)^r\). 
    \end{remark}
\end{proof}
...
\begin{align*}
    ...
    &= [t(-e^{-\lambda t})]_0^{\infty} - \int _0^{\infty} (-e^{-\lambda t})\, \mathrm{d} t = \int _0^{\infty } e^{-\lambda t} \, \mathrm{d} t = \left[ \frac{-e^{-\lambda t}}{\lambda } \right]_0^{\infty} = \frac{1}{\lambda}.     
\end{align*}
