...
\begin{itemize}
    ...
    \begin{align*}
        F_{X,Y}(x,y)
        &= \mathbb{P}(X\le x,\ Y\le y) = \mathbb{P}(X\in (-\infty,x],\ Y\in (-\infty,y]) \\
        &= \mathbb{P}(X\in (-\infty,x]) \,
           \mathbb{P}(Y\in (-\infty,y]) = F_X(x)F_Y(y).
    \end{align*}
    ...
    \begin{align*}
        ...
        +F_X(a_1)F_Y(b_1) \\
        &=
        \bigl(F_X(a_2)-F_X(a_1)\bigr)
        \bigl(F_Y(b_2)-F_Y(b_1)\bigr) \\
        &=
        \mathbb{P}(a_1<X\le a_2)\,
        \mathbb{P}(b_1<Y\le b_2) \\
        &=
        \mathbb{P}(X\in A)\mathbb{P}(Y\in B).
    \end{align*}

    Since the collection of half-open intervals \((a,b]\) generates the
    Borel \(\sigma\)-algebra on \(\mathbb{R}\), it follows from the
    monotone class theorem (or the \(\pi\)-\(\lambda\) theorem) that
    \[
    \mathbb{P}(X\in E,\ Y\in F)
    =
...
\begin{corollary}
Suppose that the random variables \(X\) and \(Y\) admit a joint density
function \(f_{X,Y}\) and marginal density functions \(f_X\) and \(f_Y\).
...
\begin{eg}
    ...
    \begin{itemize}
        ...
        \item joint density function only depends on \(X^2 + Y^2\), i.e. \(f_{X, Y}(x, y) = g(x^2 + y^2)\) for some function \(g\).      
    \end{itemize}
...
\begin{explanation}
    ...
    Now we know 
    \[
        \frac{f_X^{\prime} (x)}{f_X(x)} = Cx,
    \]
    then integrate it, we have 
    \[
        \ln f_X(x) = C^{\prime} x^2 + C^{\prime\prime}.
    \]
    Hence,
    \[
        f_X(x) = A e^{C^{\prime} x^2}
    \]
    for som constants \(A, C^{\prime}\). Now since 
    \[
        \int _{-\infty }^{\infty} f_X(x) \, \mathrm{d} x = 1, 
    \]
    we have \(C^{\prime} < 0\), so \(\exists \sigma \in \mathbb{R} _{> 0}\) s.t. \(C^{\prime} = \frac{-1}{2 \sigma ^2}\). This gives
    \[
        f_X(x) = A e^{- \frac{x^2}{2 \sigma ^2}},
    \]    
    and by \(\int _{-\infty }^{\infty} f_X(x) \, \mathrm{d} x  = 1\) and \(f_X\) is proposional to \(f_Z\) where \(Z \sim N(0, \sigma ^2)\), we have \(A = \frac{1}{\sigma \sqrt{2 \pi} }\). Hence, \(X \sim N(0, \sigma ^2)\). By symmetry, we know \(Y \sim N(0, \overline{\sigma} ^2)\). Hence, 
    \[
        f_{X, Y}(x, y) = f_X(x) f_Y(y) = \frac{1}{\sigma \overline{\sigma} 2 \pi} e^{- \frac{x^2}{2 \sigma ^2} - \frac{y^2}{2 \overline{\sigma}^2}},
    \]    
    and since this only depends on \(x^2 + y^2\), we must have \(\sigma = \overline{\sigma}\).  
