...
\begin{eg}[Grading oral exams]
    ...
    \begin{align*}
        \mathbb{P} (\text{success} ) &= \mathbb{E} [S] = \mathbb{E} [\mathbb{E} [S \mid X]] = \sum_{i=1}^n \mathbb{E} [S \mid X = i] \mathbb{P} (X = i) \\
        &= \frac{1}{n} \sum_{i=1}^n \mathbb{E} [S \mid X = i] = \frac{1}{n} \sum_{i=1}^n \mathbb{P} (\text{best student gets A+} \mid \text{they are } i \text{-th}   ) \\
...
\begin{proof}
    ...
    \] 
    This is the moment generating function of Poisson random variable with rate \(\lambda _1 + \lambda _2\), and since the MGF is unique to the distribution, \(X + Y \sim \mathrm{Poi(\lambda _1 + \lambda _2)} \).  
\end{proof}
