...
\begin{proof}
    ...
    In fact,
    \begin{align*}
        p_{67} &= \mathbb{P} (\text{happy} \mid \text{start with } \$ 67 \text{ and win first round}) \mathbb{P} (\text{win first round} \mid \text{start with } \$ 67) \\ &+ \mathbb{P} (\text{happy} \mid \text{start with } \$ 67 \text{ and lose first round}) \mathbb{P} (\text{lose first round} \mid \text{start with } \$ 67) 
    \end{align*}
...
\begin{theorem}[Second Borel-Cantelli Lemma]
    Let \((S, \mathbb{P} )\) be a probability space, and let \(E_1, E_2, \dots \) be a countably infinite sequence of \textbf{mutually independent} events. Then, if \(\sum_{n=1}^{\infty} \mathbb{P} (E_n) = \infty \), then 
    \[
...
\begin{proof}
    \begin{align*}
        \mathbb{P} (\limsup_{n \to \infty} E_n )&= \mathbb{P} \left( \lim_{n \to \infty}  \left( \bigcup_{i=n}^{\infty} E_i \right) \right) = \lim_{n \to \infty} \mathbb{P} \left( \bigcup_{i=n}^{\infty} E_i  \right) \\
...
\begin{remark}
    The convergence test for infinite product states that for any sequence \((p_i)_{i=1}^{\infty} \) with \(0 \le p_i \le 1\) for all \(i \in \mathbb{N} \), for any \(n \in \mathbb{N} \), 
    \[
        \sum_{i=n}^{\infty} p_i = \infty \implies \prod _{i=n}^{\infty} (1 - p_i) = 0. 
    \]  
    We can prove this by an inequality:
    \[
        \log (1 - x) \le -x \text{ for all } 0 \le x < 1. 
    \]
    Thus,
    \[
        \sum_{i=n}^{N} \log (1 - p_i) \le - \sum_{i=n}^N p_i \implies \log \left( \prod _{i=n}^N (1 - p_i) \right) \le -\sum_{i=n}^N p_i   
    \]
    for all \(N \ge n\). Hence, if we have \(\sum_{i=n}^{\infty} p_i = \infty  \), then \(-\sum_{i=n}^{\infty} p_i = -\infty  \). This shows 
    \[
        \log \left( \prod _{i=n}^{\infty} (1 - p_i) \right) = -\infty, 
    \]   
    so 
    \[
        \prod _{i=n}^{\infty} (1 - p_i) = 0.
    \]
\end{remark}
...
\begin{proof}
    ...
    Alternatively, 
    \begin{align*}
        \mathbb{P} (\text{getting exact } k \text{ right}) &= \mathbb{P} ( \bigcup_{\substack{R \subseteq [5] \\ \vert R \vert = k }} \left\{ \text{all answers in } R \text{ correct and all answer not in } R \text{ incorrect}    \right\}   ) \\
