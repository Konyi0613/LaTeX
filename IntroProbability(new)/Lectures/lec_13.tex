...
\begin{itemize}
    ...
    \item [(4)] For any \(i \in \mathbb{N} \), let \(T_i : S \to S\) be the map that flips the outcome of the \(i\)-th coin toss, and for \(I \subseteq \mathbb{N}\), let \(T_I: S \to S\) be the map that flips the outcomes of all toses in \(I\). For every \(E \subseteq S\), and any \(I \subseteq \mathbb{N}\), \(\mathbb{P} (T_I(E))= \mathbb{P} (E)\).              
\end{itemize}
...
\end{proposition}
\begin{proof}
    Define an equivalence relation on \(S\). Say \(w_1, w_2 \in S\) are equivalent if they only have finitely many differences, i.e. \(\exists n \in \mathbb{N}\) s.t. \((w_1)_i = (w_2)_i\) for all \(i \ge n\). Then, we can partition \(S\) into equivalence classes \([w] = \left\{ w^{\prime} \in S: w^{\prime} \sim w \right\} \). Let \(E \subseteq S\) be an event that contains one member from every equivalence class. (by Axiom of choice) 
    \begin{claim}
        \(S = \bigcupdot_{\substack{I \subseteq \mathbb{N} \\ \vert I \vert < \infty}} T_I(E) \). 
    \end{claim}        
    \begin{proof}
        For every \(w \in S\), we have that \([w] \cap E = \left\{ w^{\prime} \right\} \) for some \(w^{\prime} \sim w\). Hence, \(\exists \) finite set \(I \subseteq \mathbb{N} \) of differences. Thus, \(T_I (\left\{ w^{\prime}  \right\} ) = \left\{ w \right\} \) and \(T_I (\left\{ w^{\prime}  \right\} ) \subseteq T_I(E)\). Hence, \(w \in \bigcupdot_{\substack{I \subseteq \mathbb{N} \\ \vert I \vert < \infty}} T_I(E)\).
        ...
        \]
        Hence, \(w \sim w^{\prime\prime} \sim w^{\prime}\), so \(w = w^{\prime}\), so \(I = J\) since this means flips toss in \(I\) and toss in \(J\) getting same outcome.             
    \end{proof}
...
\begin{proof}[proof of (1)]
    \[
        \lim_{x \to \infty} F_X(x) = \mathbb{P} \left( \lim_{x \to \infty} \{X \le x\}  \right) = \mathbb{P} \left( \bigcup_{x \to \infty} \left\{ X \le x \right\}   \right) = \mathbb{P} (X \in \mathbb{R}) = 1.   
    \]
\end{proof}
...
\begin{proof}[proof of (3)]
    ...
    \[
    \lim_{n\to\infty} \mathbb{P} (X \le x_n) = \mathbb{P} \left( \lim_{n \to \infty} \left\{ X \le x_n \right\}   \right)  = \mathbb{P} \Big(\bigcap_{n=1}^{\infty} \{X \le x_n\}\Big) = \mathbb{P} (X \le x) = F_X(x).
    \]
...
\begin{definition}[Expectation and Variance of continuous random variable]
    In the discrete setting, we define \(\mathbb{E} [X] = \sum_{x_i} x_i \mathbb{P} (X = x_i) \), while in the continuous setting, we define \(\mathbb{E} [X] = \int _{-\infty }^{\infty} t f_X(t) \, \mathrm{d} t \), and we define the variance \(\Var(X) = \mathbb{E} [(X - \mathbb{E} [X])^2]\). 
\end{definition}
