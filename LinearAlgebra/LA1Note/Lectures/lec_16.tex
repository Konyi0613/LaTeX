...
\begin{eg}
    ...
    \begin{remark}
        \(f(B)(e_i) = f(B)B^{i-1}(e_1) = B^{i-1}f(B)(e_1)=0\). 
    \end{remark}
\end{eg}
...
\] and \(\mathrm{ch} _T(x) \in \mathrm{Ann}_T(V) \)(Cayley-Hamilton Theorem). 
...
\begin{definition}
    ...
    \begin{align*}
        ...
        &= \left\{ f(x) \in F[x] \mid f(T)(V) \subseteq W \right\}.    
    \end{align*}  
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \[
            \mathrm{Ann}_T(V / W) = F[x] \cdot p(x), 
        \] and WLOG we can suppose \(p(x) = (x - \lambda _1)q(x)\) since \(m_T(x) \in \mathrm{Ann}_T(V / W) \), and we can check that \(p(x)\) cannot be a constant polynomial, otherwise \(V = W\), which is a contradiction. Thus, there exists \(v \in V\) s.t. \(q(T)(v) \notin W\). Set 
        \[
