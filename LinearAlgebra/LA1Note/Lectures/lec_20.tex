...
\begin{proof}[proof of (i)]
    ...
    Thus, 
    \[
        \ker g_i(T) \subseteq \Im \xi _i h_i (T) \subseteq \Im h_i(T).
    \]  
\end{proof}  
\begin{proof}[proof of (ii)]
    It follows from (i) since we can show \(\Im h_i(T) = \ker (g_i(T))\). Note that 
    \[
...
\]  
is a prime decomposition, i.e. every \(p_i\) monic and irreducible and \(p_i \neq p_j\) for each \(i \neq j\). Then, we have \(V = \bigoplus V_i\) where \(V_i = \ker \left( p_i(T)^{m_i} \right) \). This is the primary decomposition theorem.    
